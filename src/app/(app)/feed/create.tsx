...
import { Image as ImageIcon, FileUp, Check, Sparkles, Lock } from "lucide-react-native";
...
export default function CreateProductScreen() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      formData.append("pushTo", "community");
          ...
          <View className="gap-2">
            {activeSlots.map((s) => {
              ...
              return (
                <Pressable
                  key={s.slot}
                  onPress={() => pickSlotFile(s.slot)}
                  className="flex-row items-center gap-3 bg-navy-800 border border-navy-600 rounded-2xl px-4 py-3.5"
                >
